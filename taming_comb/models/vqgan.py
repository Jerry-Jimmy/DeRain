...
#        self.assign_adain_params(adain_params, self.decoder_b)
    ...
    def forward(self, input, label):
        if(label == 1):
            quant, diff, _ = self.encode(input, label)     #uant, diff, _, style_encoded = self.encode(input, label)
            dec = self.decode_a(quant)
        else:
            quant, diff, _ = self.encode(input, label)
...
class VQModelCrossGAN_ADAIN(VQModel_ADAIN):
    ...
    def forward(self, input, label, cross=False, s_given=False,use_checkpoint=True):
        input = Variable(input,requires_grad=True)
        if use_checkpoint:
            quant, diff= checkpoint(self.encode, input)
            ...
            if(label == 1):
                if cross == False:
                    output = checkpoint(self.decode_a , quant)
                else:
                    s = s_given
                    output = checkpoint(self.decode_b ,quant)
            else:
                if cross == False:
                    output = checkpoint(self.decode_b , quant)
                else:
            ...
            if(label == 1):
                if cross == False:
                    output = self.decode_a(quant)
                else:
                    s = s_given
                    output = self.decode_b(quant)
            else:
                if cross == False:
                    output = self.decode_b(quant)
                else:
